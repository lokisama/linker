...
.controller('UserIndexController', ['$scope', '$http', '$state', '$mdMedia','userApi', 'markdownDialog', 'alertDialog', 'payDialog', 'qrcodeDialog', '$interval', '$localStorage', 'changePasswordDialog', 'payByGiftCardDialog', 'subscribeDialog', '$q',
  ($scope, $http, $state, $mdMedia,userApi, markdownDialog, alertDialog, payDialog, qrcodeDialog, $interval, $localStorage, changePasswordDialog, payByGiftCardDialog, subscribeDialog, $q) => {
    $scope.setTitle('首页');
    ...
    $scope.getServerPortData = (account, serverId) => {
      ...
      userApi.getServerPortData(account, serverId).then(success => {
        ...
        $scope.flow.type = account.type;
        $scope.flow.limit = account.data.limit;
        $scope.flow.expire = account.data.expire;
        $scope.flow.serverPortFlow = success.flow;
        $scope.flow.maxFlow = maxFlow;
        $scope.flow.serverId = serverId;
